      ...
      _toggle_toolbars_scene_act(main_window->findChild<QAction*>("toggleListboxToolbars"))
{
	...
	config_t *const user_config = obs_frontend_get_user_config();
	config_set_default_bool(user_config, "SceneTreeView", "ShowSceneIcons", false);
	config_set_default_bool(user_config, "SceneTreeView", "ShowFolderIcons", false);
	...
	// OBS v32: Use obs_frontend_get_app_config() for app-level settings
	const bool show_icons = config_get_bool(obs_frontend_get_app_config(), "BasicWindow", "ShowListboxToolbars");
...
//	QMenu order(QTStr("Basic.MainMenu.Edit.Order"), this);
	...
	// Qt6: Use modern signal/slot syntax instead of SLOT macro
	QAction *addSceneAction = popup.addAction(obs_module_text("SceneTreeView.AddScene"));
	QObject::connect(addSceneAction, &QAction::triggered, [main_window]() {
		QMetaObject::invokeMethod(main_window, "on_actionAddScene_triggered");
	});

	popup.addAction(obs_module_text("SceneTreeView.AddFolder"),
	                this, &ObsSceneTreeView::on_stvAddFolder_clicked);

	if(item)
	{
		if(item->type() == StvItemModel::SCENE)
		{
			// Qt6: Use modern signal/slot syntax instead of SIGNAL/SLOT macros
			QAction *copyFilters = new QAction(QTStr("Copy.Filters"), this);
			copyFilters->setEnabled(false);
			QObject::connect(copyFilters, &QAction::triggered, [main_window]() {
				QMetaObject::invokeMethod(main_window, "SceneCopyFilters");
			});
...
//			    !obs_weak_source_expired(copyFiltersSource));			// Cannot use (we can't check copyFiltersSource, as it's a private member of OBSBasic)
			QObject::connect(pasteFilters, &QAction::triggered, [main_window]() {
				QMetaObject::invokeMethod(main_window, "ScenePasteFilters");
			});

			popup.addSeparator();
			QAction *duplicateAction = popup.addAction(QTStr("Duplicate"));
			QObject::connect(duplicateAction, &QAction::triggered, [main_window]() {
				QMetaObject::invokeMethod(main_window, "DuplicateSelectedScene");
			});
			popup.addAction(copyFilters);
			popup.addAction(pasteFilters);
			popup.addSeparator();
			QAction *rename = popup.addAction(QTStr("Rename"));
			QObject::connect(rename, &QAction::triggered, this->_stv_dock.stvTree, &StvItemView::EditSelectedItem);
			QAction *removeAction = popup.addAction(QTStr("Remove"));
			QObject::connect(removeAction, &QAction::triggered, [main_window]() {
				QMetaObject::invokeMethod(main_window, "RemoveSelectedScene");
			});
			popup.addSeparator();
...
//			popup.addMenu(sceneProjectorMenu);
			...
			QObject::connect(sceneWindow, &QAction::triggered, [main_window]() {
				QMetaObject::invokeMethod(main_window, "OpenSceneWindow");
			});

			popup.addAction(sceneWindow);
			QAction *screenshotAction = popup.addAction(QTStr("Screenshot.Scene"));
			QObject::connect(screenshotAction, &QAction::triggered, [main_window]() {
				QMetaObject::invokeMethod(main_window, "ScreenshotScene");
			});
			popup.addSeparator();
			QAction *filtersAction = popup.addAction(QTStr("Filters"));
			QObject::connect(filtersAction, &QAction::triggered, [main_window]() {
				QMetaObject::invokeMethod(main_window, "OpenSceneFilters");
			});
...
QMenu *ObsSceneTreeView::CreatePerSceneTransitionMenu(QMainWindow *main_window)
{
	...
	connect(duration, &QSpinBox::valueChanged, setDuration);
